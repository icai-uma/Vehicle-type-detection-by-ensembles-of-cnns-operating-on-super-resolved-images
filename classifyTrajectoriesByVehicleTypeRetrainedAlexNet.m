function classifyTrajectoriesByVehicleTypeRetrainedAlexNet()
...
% Load the ReTrained AlexNet
params.TrainedCNNDirectory = strcat(params.MatlabProjectsDirectory,'trained_cnn/proyectos/SOM_CutObjects/');
load(strcat(params.TrainedCNNDirectory,'trainedNetClassifyVehiclesType.mat'));
...
for NdxRepeticion=1:10
    ...
    for NdxClase=1:numVehicleTypes
        ...
        for NdxMuestra=1:size(MuestrasTestRep,2)
            ...
            distancesTestItemsNeuronsCarac{NdxMuestra,NdxClase}=distancesItemNeurons;
        ...
        for NdxMuestra=1:size(noLabeledData,2)
            ...
            distancesNoLabeledItemsNeuronsCarac{NdxMuestra,NdxClase}=distancesItemNeurons;
            %save the min distance
    ...
    for NdxMuestra=1:size(MuestrasTest{NdxRepeticion},2)
        for NdxClase=1:numVehicleTypes
            for NdxNeuro=1:MaxUnits 
                distancesTestItemsNeurons{NdxMuestra,NdxClase}(NdxNeuro) = sum(distancesTestItemsNeuronsCarac{NdxMuestra,NdxClase}(NdxNeuro,:).^2);
                
            end
            minDistanceTestItemClass(NdxMuestra,NdxClase) = min(distancesTestItemsNeurons{NdxMuestra,NdxClase});
        end
    ...
    for NdxMuestra=1:size(noLabeledData,2)
        for NdxClase=1:numVehicleTypes
            for NdxNeuro=1:MaxUnits 
                distancesNoLabeledItemsNeurons{NdxMuestra,NdxClase}(NdxNeuro) = sum(distancesNoLabeledItemsNeuronsCarac{NdxMuestra,NdxClase}(NdxNeuro,:).^2);
            end
            minDistanceNoLabeledItemClass(NdxMuestra,NdxClase) = min(distancesNoLabeledItemsNeurons{NdxMuestra,NdxClase});
        end
...
PdfFileName=sprintf('model_training_width_height.pdf');
...
PdfFileName=sprintf('results_training_width_height.pdf');
set(gcf,'PaperUnits','centimeters');
set(gcf,'PaperOrientation','portrait');
set(gcf,'PaperPositionMode','manual');
set(gcf,'PaperSize',[12 11]);
...
PdfFileName=sprintf('model_test_area_perimeter.pdf');
set(gcf,'PaperUnits','centimeters');
set(gcf,'PaperOrientation','portrait');
set(gcf,'PaperPositionMode','manual');
set(gcf,'PaperSize',[12 11]);
...
PdfFileName=sprintf('results_test_area_perimeter.pdf');
set(gcf,'PaperUnits','centimeters');
set(gcf,'PaperOrientation','portrait');
set(gcf,'PaperPositionMode','manual');
set(gcf,'PaperSize',[12 11]);
...
%para cada .mat se guardan los resultados predicho
