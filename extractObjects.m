function [obj_detectados,flag] = extractObjects(objetos)
...
for i=1:N
    ...
    obj_detectados(cont).Orientation = stats(i).Orientation;
    obj_detectados(cont).posibleError = 0;
    obj_detectados(cont).splitted = 0;
    obj_detectados(cont).ColorToDraw = [1 1 1];
    obj_detectados(cont).OrientationNeuronAssociated = -1;
...
%     X = ListaPixeles_y_ColoresObjeto(obj_detectados(cont),frame);
    ...
    obj_detectados(cont).type  = [];
    obj_detectados(cont).idxType  = [];
    
    obj_detectados(cont).imagen_normalizada = [];
